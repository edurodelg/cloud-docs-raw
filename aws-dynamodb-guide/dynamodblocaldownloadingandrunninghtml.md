...
fetched_at: 2026-02-05T08:25:45.210363
---
...
-
Migration from DynamoDB local V2.x to V3.x requires updating import statements from

`com.amazonaws.services.dynamodbv2`

to`software.amazon.dynamodb`

and updating Maven dependencies for Maven users. -
If you're migrating an application that uses the SDK for Java v1.x to the SDK for Java 2.x, follow the steps for

[AWS SDK for Java 2.x](https://docs.aws.amazon.com/sdk-for-java/latest/developer-guide/migration.html).
...
-
Download DynamoDB local for free from one of the following locations.

Download Links Checksums ###### Important
...
, enter this command.`java -Djava.library.path=./DynamoDBLocal_lib -jar DynamoDBLocal.jar -help`
...
command of the AWS CLI to set up credentials. For more information, see[Using the AWS CLI](./AccessingDynamoDB.html#Tools.CLI). -
Start writing applications. To access DynamoDB running locally with the AWS CLI, use the

`--endpoint-url`

parameter. For example, use the following command to list DynamoDB tables.`aws dynamodb list-tables --endpoint-url http://localhost:8000`
...
Copy the following code to a file and save it as
...
This docker-compose.yml script creates an

`app-node`

container and a`dynamodb-local`

container. The script runs a command in the`app-node`

container that uses the AWS CLI to connect to the`dynamodb-local`

container and describes the account and table limits.To use with your own application image, replace the

`image`

value in the example below with that of your application.`version: '3.8' services: dynamodb-local: command: "-jar DynamoDBLocal.jar -sharedDb -dbPath ./data" image: "amazon/dynamodb-local:latest" container_name: dynamodb-local ports: - "8000:8000" volumes: - "./docker/dynamodb:/home/dynamodblocal/data" working_dir: /home/dynamodblocal app-node: image:`

`location-of-your-dynamodb-demo-app:latest`

container_name: app-node ports: - "8080:8080" depends_on: - "dynamodb-local" links: - "dynamodb-local" environment: AWS_ACCESS_KEY_ID: 'DUMMYIDEXAMPLE' AWS_SECRET_ACCESS_KEY: 'DUMMYEXAMPLEKEY' REGION: 'eu-west-1'###### Note
...
the SDK for Java 2.x, follow the steps for [AWS SDK for Java 2.x](https://docs.aws.amazon.com/sdk-for-java/latest/developer-guide/migration.html).
...
-
Download and install Apache Maven. For more information, see
...
a few different ways. For more information, see [Getting started with AWS CloudShell](https://docs.aws.amazon.com/cloudshell/latest/userguide/getting-started.html).
...
-
Launch AWS CloudShell from the console interface, choose an available AWS Region, and switch to your preferred shell, such as Bash, PowerShell, or Z shell.

-
To choose an AWS Region, go to the

**Select a Region**menu and select a[supported AWS Region](https://docs.aws.amazon.com/cloudshell/latest/userguide/supported-aws-regions.html). (Available Regions are highlighted.) -
From the AWS Management Console, launch AWS CloudShell by choosing one of the following options:

-
On the navigation bar, choose the

**AWS CloudShell**icon. -
In the

**Search**box, enter the word CloudShell, and then choose**CloudShell**. -
In the

**Recently visited**widget, choose**CloudShell**. -
From the console toolbar, choose

**CloudShell**.

-
-
To run DynamoDB local in AWS CloudShell you can use the

`dynamodb-local`

alias. You can specify additional command line options for changing DynamoDB local settings. See[DynamoDB local usage notes](./DynamoDBLocal.UsageNotes.html)for available options.###### Note

To run DynamoDB local in the background, run DynamoDB local in AWS CloudShell using:

`dynamodb-local &`

. -
To access DynamoDB running locally in AWS CloudShell with the AWS CLI, use the
