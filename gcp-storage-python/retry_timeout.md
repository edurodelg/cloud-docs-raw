...
fetched_at: 2026-02-08T01:07:38.148872
---
...
the first byte. Please refer to the [requests documentation](https://requests.readthedocs.io/en/latest/user/advanced/#timeouts) for details.
...
policy, which retries any API request which returns a “transient” error.`DEFAULT_RETRY`

For API requests which are idempotent only if the blob has the same “generation”, the library uses its

policy, which retries API requests which returns a “transient” error, but only if the original request includes a`DEFAULT_RETRY_IF_GENERATION_SPECIFIED`

`generation`

or`ifGenerationMatch`

header.For API requests which are idempotent only if the bucket or blob has the same “metageneration”, the library uses its

policy, which retries API requests which returns a “transient” error, but only if the original request includes an`DEFAULT_RETRY_IF_METAGENERATION_SPECIFIED`

`ifMetagenerationMatch`

header.For API requests which are idempotent only if the bucket or blob has the same “etag”, the library uses its

policy, which retries API requests which returns a “transient” error, but only if the original request includes an`DEFAULT_RETRY_IF_ETAG_IN_JSON`
...
by calling it with a`DEFAULT_RETRY`
...
to enable retries; the passed object will define retriable response codes and errors, as well as configuring backoff and retry interval options. E.g.:`google.api_core.retry.Retry`
...
- You can pass an instance of
, which wraps a`google.cloud.storage.retry.ConditionalRetryPolicy`
...
```[is_etag_in_data](https://docs.cloud.google.com/python/docs/reference/storage/latest/google.cloud.storage.retry.html)
def is_retryable(exc):
... # as above
my_retry_policy = Retry(predicate=is_retryable)
