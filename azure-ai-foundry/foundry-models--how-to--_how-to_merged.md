---
merged_at: 2026-01-25T15:32:35.869085
merged_files: 15
---
...
<!-- DOCUMENTO FUSIONADO: configure-marketplace.md -->
<!-- URL ORIGINAL: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-models/how-to/configure-marketplace -->
...
<!-- DOCUMENTO FUSIONADO: quickstart-github-models.md -->
<!-- URL ORIGINAL: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-models/how-to/quickstart-github-models -->
...
<!-- DOCUMENTO FUSIONADO: generate-responses.md -->
<!-- URL ORIGINAL: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-models/how-to/generate-responses -->
...
[Create a project for Microsoft Foundry](../../how-to/create-projects?view=foundry-classic).Your Foundry project's endpoint URL, which is of the form
...
model used in this article. If you don't have a deployment already, see[Add and configure Foundry Models](create-model-deployments?view=foundry-classic)to a model deployment to your resource.
...
`pip install azure-identity pip install openai pip install --pre azure-ai-projects>=2.0.0b1`
...
This section lists some of the Foundry Models that are supported for use with the Responses API. For the Azure OpenAI models that are supported, see [Available Azure OpenAI models](../../agents/concepts/model-region-support?view=foundry-classic#available-models).
...
<!-- DOCUMENTO FUSIONADO: configure-project-connection.md -->
<!-- URL ORIGINAL: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-models/how-to/configure-project-connection -->
...
<!-- DOCUMENTO FUSIONADO: use-chat-completions.md -->
<!-- URL ORIGINAL: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-models/how-to/use-chat-completions -->
...
<!-- DOCUMENTO FUSIONADO: deploy-foundry-models.md -->
<!-- URL ORIGINAL: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-models/how-to/deploy-foundry-models -->
...
<!-- DOCUMENTO FUSIONADO: quickstart-ai-project.md -->
<!-- URL ORIGINAL: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-models/how-to/quickstart-ai-project -->
...
<!-- DOCUMENTO FUSIONADO: create-model-deployments.md -->
<!-- URL ORIGINAL: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-models/how-to/create-model-deployments -->
...
<!-- DOCUMENTO FUSIONADO: monitor-models.md -->
<!-- URL ORIGINAL: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-models/how-to/monitor-models -->
...
<!-- DOCUMENTO FUSIONADO: use-foundry-models-claude.md -->
<!-- URL ORIGINAL: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-models/how-to/use-foundry-models-claude -->
...
**Extended thinking**: Extended thinking gives Claude enhanced reasoning capabilities for complex tasks.**Image and text input**: Strong vision capabilities that enable the models to process images and return text outputs for analyzing and understanding charts, graphs, technical diagrams, reports, and other visual assets.**Code generation**: Advanced thinking that includes code generation, analysis, and debugging for Claude Sonnet 4.5 and Claude Opus 4.1.
...
- An Azure subscription with a valid payment method. If you don't have an Azure subscription, create a
[paid Azure account](https://azure.microsoft.com/pricing/purchase-options/pay-as-you-go)to begin. - Access to Microsoft Foundry with appropriate permissions to create and manage resources.
- A
[Microsoft Foundry project](../../how-to/create-projects?view=foundry-classic)created in one of the supported regions:**East US2**and**Sweden Central**. [Foundry Models from partners and community](../concepts/models-from-partners?view=foundry-classic)require access to**Azure Marketplace**to create subscriptions. Ensure you have the[permissions required to subscribe to model offerings](configure-marketplace?view=foundry-classic).
...
[Anthropic SDKs](https://docs.claude.com/en/api/client-sdks)and the following Claude APIs:[Messages API](https://docs.claude.com/en/api/messages)to send a structured list of input messages with text and/or image content, and the model generates the next message in the conversation.[Token Count API](https://docs.claude.com/en/api/messages-count-tokens)to count the number of tokens in a message.[Files API](https://docs.claude.com/en/api/files-create)to upload and manage files to use with the Claude API without having to re-upload content with each request.[Skills API](https://docs.claude.com/en/api/skills/create-skill)to create custom skills for Claude AI.
...
. - Your target URI from your deployment details, which is of the form
...
**Install the Azure Identity client library**: You need to install this library to use the`DefaultAzureCredential`

. Authorization is easiest when you use`DefaultAzureCredential`

, as it finds the best credential to use in its running environment.`pip install azure.identity`
...
.`export AZURE_CLIENT_ID="<AZURE_CLIENT_ID>" export AZURE_TENANT_ID="<AZURE_TENANT_ID>" export AZURE_CLIENT_SECRET="<AZURE_CLIENT_SECRET>"`

**Install dependencies:**Install the Anthropic SDK by using pip (requires: Python >=3.8).`pip install -U "anthropic"`

**Run a basic code sample:**This sample completes the following tasks:- Creates a client with the Anthropic SDK, using Microsoft Entra ID authentication.
...
**Run a basic code sample:**This sample completes the following tasks:- Creates a client with the Anthropic SDK by passing your API key to the SDK's configuration. This authentication method lets you interact seamlessly with the service.
- Makes a basic call to the Messages API. The call is synchronous.
...
[Microsoft Agent Framework](/en-us/agent-framework/user-guide/agents/agent-types/anthropic-agent)supports creating agents that use Claude models.- You can build custom AI agents with the
...
**1 million token context window:**An extended context window.**Agent skills:**Extend Claude's capabilities with Skills.**Citations:**Ground Claude's responses in source documents.**Context editing:**Automatically manage conversation context with configurable strategies.**Extended thinking:**Enhanced reasoning capabilities for complex tasks.**PDF support:**Process and analyze text and visual content from PDF documents.**Prompt caching:**Provide Claude with more background knowledge and example outputs to reduce costs and latency.

Some of the **Tools** that Foundry supports are:

**MCP connector:**Connect to remote MCP servers directly from the Messages API without a separate MCP client.**Memory:**Store and retrieve information across conversations. Build knowledge bases over time, maintain project context, and learn from past interactions.**Web fetch:**Retrieve full content from specified web pages and PDF documents for in-depth analysis.
...
**Implement retry logic**: Handle 429 responses with exponential backoff**Batch requests**: Combine multiple prompts when possible**Monitor usage**: Track your token consumption and request patterns**Use appropriate models**: Choose the right Claude model for your use case

## Responsible AI considerations

When using Claude models in Foundry, consider these responsible AI practices:

Configure AI content safety during model inference, as Foundry doesn't provide built-in content filtering for Claude models at deployment time. To learn how to create and use content filters, see
...
**Claude Opus 4.5**: For best performance across coding, agents, computer use, and enterprise workflows**Claude Sonnet 4.5**: For balanced performance and capabilities, production workflows**Claude Haiku 4.5**: For speed and cost optimization, high-volume processing**Claude Opus 4.1**: For complex reasoning and enterprise applications

#### Prompt engineering

**Clear instructions**: Provide specific and detailed prompts**Context management**: Effectively use the available context window**Role definitions**: Use system messages to define the assistant's role and behavior**Structured prompts**: Use consistent formatting for better results

#### Cost optimization

**Token management**: Monitor and optimize token usage**Model selection**: Use the most cost-effective model for your use case**Caching**: Implement[explicit prompt caching](https://docs.claude.com/en/docs/build-with-claude/prompt-caching#continuing-a-multi-turn-conversation)where appropriate**Request batching**: Combine multiple requests when possible


---

<!-- DOCUMENTO FUSIONADO: configure-content-filters.md -->
<!-- URL ORIGINAL: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-models/how-to/configure-content-filters -->
...
<!-- DOCUMENTO FUSIONADO: inference.md -->
<!-- URL ORIGINAL: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-models/how-to/inference -->
...
<!-- DOCUMENTO FUSIONADO: quickstart-create-resources.md -->
<!-- URL ORIGINAL: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-models/how-to/quickstart-create-resources -->
...
<!-- DOCUMENTO FUSIONADO: configure-entra-id.md -->
<!-- URL ORIGINAL: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-models/how-to/configure-entra-id -->
...
<!-- DOCUMENTO FUSIONADO: use-chat-reasoning.md -->
<!-- URL ORIGINAL: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-models/how-to/use-chat-reasoning -->
