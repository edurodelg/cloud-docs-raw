...
field of an [InvokeModel](https://docs.aws.amazon.com/bedrock/latest/APIReference/API_runtime_InvokeModel.html) request.
...
-
embedding – An array that represents the embedding vector of the input you provided. This will always be type
...
-
This field will always appear. Even if you don't specify

`embeddingTypes`

in your input, there will still be “float”. Example:`"embeddingsByType": {"float": [float,...]}`
...
- G1 Request
-
The only available field is

`inputText`

, in which you can include text to convert into an embedding.`{ "inputText": string }`
...
of the response contains the following fields.`{ "embedding": [float, float, ...], "inputTextTokenCount": int }`
...
-
When using Titan Text Embeddings V2, the

`embedding`

field is not in the response if the`embeddingTypes`
