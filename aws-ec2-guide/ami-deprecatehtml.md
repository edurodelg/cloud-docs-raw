...
fetched_at: 2026-01-28T07:10:04.240567
---
...
services cannot use it, you must [deregister](./deregister-ami.html) it.

After an AMI is deprecated:

-
For AMI users, the deprecated AMI does not appear in [DescribeImages](https://docs.aws.amazon.com/AWSEC2/latest/APIReference/API_DescribeImages.html)
API calls unless you specify its ID or specify that deprecated AMIs must appear. AMI
owners continue to see deprecated AMIs in [DescribeImages](https://docs.aws.amazon.com/AWSEC2/latest/APIReference/API_DescribeImages.html)
API calls.
...
[deregistering](./deregister-ami.html) it.
...
more information, see [Check when an Amazon EC2 AMI was last used](./ami-last-launched-time.html).
...
accounts](./sharingamis-explicit.html).
...
-
Select the AMI, and then choose **Actions**, **Manage AMI
Deprecation**. You can select multiple AMIs to set the
same deprecation date of several AMIs at once.

-
Select the **Enable** checkbox, and then enter the deprecation date and
time.
...
-
Check the **Deprecation time** field (if you selected the checkbox
...
specify the `--include-deprecated`
...
-
Clear the **Enable** checkbox, and then choose
**Save**.
