...
fetched_at: 2026-02-05T08:20:21.646870
---
...
tools, see [Set up the Amazon EC2 AMI tools](./set-up-ami-tools.html).

For information about your access keys, see [Managing access keys for
...
. If you installed the AMI tools from the RPM or ZIP file in[Set up the Amazon EC2 AMI tools](./set-up-ami-tools.html), the certificates are located at`$EC2_AMITOOL_HOME/etc/ec2/amitools/`
...
—The root file system device, as seen by the instance -
`root`

—The root file system device, as seen by the kernel -
`swap`

—The swap device, as seen by the instance -
`ephemeralN`

—The Nth instance store volume
...
[register-image](https://docs.aws.amazon.com/cli/latest/reference/ec2/register-image.html)to set the kernel.Required: No
...
[register-image](https://docs.aws.amazon.com/cli/latest/reference/ec2/register-image.html)to set the RAM disk if required.Required: No
...
option. To include some of these files, use the
`--include`
...
option. When exclude is specified, the directories and subdirectories listed with the parameter will not be bundled with the volume.Required: No
...
but the instance metadata is not accessible.Required: No
...
to exist on the cloned image. This option allows you to specify a path to an alternative grub configuration file, which will then be copied over the defaults (if present).Required: No
...
*token*-
The delegation token to pass along to the AWS request. For more information, see

[Temporary security credentials](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_temp.html)in the*IAM User Guide*.Required: Only when you are using temporary security credentials.

Default: The value of the

`AWS_DELEGATION_TOKEN`

environment variable (if set). `--region`
...
operations to perform automated mappings.Required: Required if you're not providing the
...
options used for automatic mapping. `--ec2cert`
...
. If you installed the AMI tools from the ZIP file in[Set up the Amazon EC2 AMI tools](./set-up-ami-tools.html), the certificates are located at`$EC2_AMITOOL_HOME/etc/ec2/amitools/`
...
[User provided kernels](https://docs.aws.amazon.com/linux/al2/ug/UserProvidedKernels.html)in the*Amazon Linux 2 User Guide*.Required: No
...
ls-l unbundled`total 1025008 -rw-r--r-- 1 root root 1048578048 Aug 25 23:46 image.img`
...
Uploads the bundle for an Amazon S3-backed Linux AMI to Amazon S3 and sets the appropriate access
control lists (ACLs) on the uploaded objects. For more information, see [Create an Amazon S3-backed AMI](./creating-an-ami-instance-store.html).
...
environment variable (if set). `-m, --manifest`
...
flag is the only way to target that specific location restraint.The Amazon S3 endpoint service URL.
...
flag is the only way to target that specific location restraint.Default:
...
This example uploads the bundle specified by the `image.manifest.xml`
