...
-
You can't copy a paravirtual (PV) AMI to a Region that does not support PV AMIs. For more information, see

[Virtualization types](./ComponentsAMIs.html#virtualization_types).

## Cross-account copying

If an AMI from another AWS account is [shared with your AWS account](./sharingamis-explicit.html), you can copy the shared AMI. This is
...
Amazon EBS snapshot with other AWS accounts](https://docs.aws.amazon.com/ebs/latest/userguide/ebs-modifying-snapshot-permissions.html) in the
*Amazon EBS User Guide*, and for S3 buckets, see [Identity and access management for Amazon S3](https://docs.aws.amazon.com/AmazonS3/latest/userguide/security-iam.html) in the
...
Time-based copies](https://docs.aws.amazon.com/ebs/latest/userguide/time-based-copies.html) in the *Amazon EBS User Guide*.
...
action applies only to Amazon EBS-backed AMIs.
...
action to change the target AMI's encryption
state.
...
For more information about copying AMIs with encrypted snapshots, see [Use encryption with EBS-backed AMIs](./AMIEncryption.html).