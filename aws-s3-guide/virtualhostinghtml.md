...
fetched_at: 2026-01-25T12:20:23.317393
---
...
.
For a complete list of Amazon S3 Regions and endpoints, see `bucket-name`

.s3.`region-code`

.amazonaws.com[Amazon S3 endpoints and quotas](https://docs.aws.amazon.com/general/latest/gr/s3.html) in the *Amazon Web Services General Reference*.
...
Deprecation Plan](https://aws.amazon.com/blogs/aws/amazon-s3-path-deprecation-plan-the-rest-of-the-story/) on the
...
Currently, Amazon S3 supports both virtual-hosted–style and path-style URL access in all AWS Regions. However, path-style
...
see [Amazon S3 Path Deprecation Plan – The Rest of the Story](https://aws.amazon.com/blogs/aws/amazon-s3-path-deprecation-plan-the-rest-of-the-story/) in the
...
CloudFront distribution. For more information, see [Website endpoints](./WebsiteEndpoints.html) and [ Deploy a React-based single-page application to Amazon S3 and CloudFront](https://docs.aws.amazon.com/prescriptive-guidance/latest/patterns/deploy-a-react-based-single-page-application-to-amazon-s3-and-cloudfront.html) in the
...
header is omitted or its value is`s3.`

, the bucket for the request will be the first slash-delimited component of the Request-URI, and the key for the request will be the rest of the Request-URI. This is the ordinary method, as illustrated by the first and second examples in this section. Omitting the`region-code`
...
, the bucket name is the leading component of the`region-code`

.amazonaws.com`Host`

header's value up to`.s3.`

. The key for the request is the Request-URI. This interpretation exposes buckets as subdomains of`region-code`
...
For information about DNS-compatible names, see [Limitations](#VirtualHostingLimitations). For more information about
keys, see [Keys](./Welcome.html#BasicsKeys).
...
.
For more information, see `bucket-name`

.s3.us-east-1.amazonaws.com[Customizing Amazon S3 URLs with CNAME
records](#VirtualHostingCustomURLs).
...
.amazonaws.com`http://images.example.com/`
...
endpoints](https://docs.aws.amazon.com/AmazonS3/latest/API/RESTAPI.html) in the *Amazon S3 API Reference*. To create a static website by using a custom domain, see [Tutorial: Configuring a static website using a
custom domain registered with Route 53](./website-hosting-custom-domain-walkthrough.html).
...
information. For a complete list of Amazon S3 Regions and endpoints, see [Amazon S3 endpoints and quotas](https://docs.aws.amazon.com/general/latest/gr/s3.html) in the *Amazon Web Services General Reference*.
...
see [Making requests ](https://docs.aws.amazon.com/AmazonS3/latest/API/MakingRequests.html) in the *Amazon S3 API
Reference*.
...
HTTP response code 301 Permanent Redirect error and a message that indicates the
...
an HTTP 301 Permanent Redirect error.`bucket-name`