...
fetched_at: 2026-01-30T23:51:43.403951
---
...
exports themselves. Code *may* rename imports (```
import {SomeThing as
SomeOtherThing}
```
...
s used to be called internal modules and used to use the`module`
...
: a declaration (i.e. not an expression) using the`function`

keywordfunction expression

: an expression, typically used in an assignment or passed as a parameter, using the`function`
...
(but this is [discouraged](#rebinding-this)), or for
...
[MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Deprecated_and_obsolete_features)) - New ECMAScript features that are not yet standardized
- Avoid using features that are in current TC39 working draft or currently
in the
[proposal process](https://tc39.es/process-document/) - Use only ECMAScript features defined in the current ECMA-262 specification
...
prefix for optional parameters.- For accessors, see
[accessor rules](#getters-and-setters-accessors)below.
...
Note: If you're creating an alias just to expose it to a template in your framework of choice, remember to also apply the proper
...
s can be keyed by—and`Set`

s can contain—types other than`string`
...
,
consider one of:#### Providing a more specific type
...
is better for dictionary-like objects, and provides better type safety by being explicit about the type`T`

of contained values (which may itself be`unknown`
...
have slightly different meaning from the corresponding primitive types`string`
...
for a type that include everything except`null`

and`undefined`

, or lowercase`object`

to further exclude the other primitive types (the three mentioned above, plus`symbol`
...
**Code should be maintainable in the long term.**Code usually lives longer than the original author works on it, and the TypeScript team must keep all of Google working into the future.
...
**Code reviewers should be focused on improving the quality of the code, not enforcing arbitrary rules.**If it's possible to implement your rule as an automated check that is often a good sign. This also supports principle 3.
