...
fetched_at: 2026-02-04T00:27:00.619945
---
...
(e.g., [ k8s](https://pkg.go.dev/k8s.io/client-go/kubernetes),
...
local variable names. For example, `usercount`

is a better package name than
`count`

, since `count`

is a commonly used variable name.
...
suffix for unit tests that only exercise the exported API
of a package (package `testing`

calls these
[“black box tests”](https://pkg.go.dev/testing)). For example, a package
`linkedlist`

must define its black box unit tests in a package named
`linkedlist_test`

(not `linked_list_test`

)

Using underscores and the `_test`

suffix for packages that specify
...
Avoid uninformative package names like `util`
...
, and so on that would tempt users of the package to
...
Do not use non-MixedCaps constant names or constants with a `K`
...
. As a general rule, identifiers (e.g., `ID`
...
because it contains `XML`

and `API`

), each letter within a given initialism should have the same case,
...
), the initialism should appear as it would in standard prose,
unless you need to change the first letter for the sake of `ddos`
...
Function and method names should not use a `Get`

or `get`

prefix, unless the
underlying concept uses the word “get” (e.g. an HTTP GET). Prefer starting the
name with the noun directly, for example use `Counts`
...
call, a different word like `Compute`
...
A name that might be perfectly clear (e.g., `c`

for a counter) within a small
...
that might normally be reserved for very small scopes may remain
perfectly clear even if the scope is very large. In this case, a single word
`database`

is likely acceptable based on the size of the scope, but is not
required as `db`
...
are a good starting point.`userCount`
...
, particularly for exported names.`userCount`

is a better name than `numUsers`
...
is a better name than `userSlice`

.`ageString`

and use `age`

for the parsed value.`UserCount`

method, a local variable called
`userCount`

is probably redundant; `count`
...
are just as
readable.Single-letter variable names can be a useful tool to minimize
...
itself, the canonical name for the constructor is `New`
...
or with the underlying representation:
...
However, were the result parameters written as `(Context, func())`
...
[above, the real code uses a]`WithTimeout`

[instead of a raw]`CancelFunc`

`func()`

in the result parameter list and requires little effort to document.
...
packages have a slightly different form, where the name of
the `go_binary`

rule in the BUILD file takes the place of the package name.
...
with only the comment and the package clause.
...
their names, and their local names must have a `pb`
...
uninformative name (e.g. `util`

or `v1`

) Do this sparingly: do not rename the
...
name that you want to use (e.g. `url`

, `ssh`

) and you wish to rename the
package, the preferred way to do so is with the `pkg`
...
Import for [side-effects](https://go.dev/doc/effective_go#blank_import)
...
in a source file which uses the `//go:embed`
...
to signal that a function can fail. By convention, `error`
...
error is the idiomatic way to signal a successful operation
...
Exported functions that return errors should return them using the `error`

type.
Concrete error types are susceptible to subtle bugs: a concrete `nil`
...
while the function was running.Error strings should not be capitalized (unless beginning with an exported name, a proper noun or an acronym) and should not end with punctuation. This is because error strings usually appear within other context before being printed to the user.
...
handle it. It is not usually appropriate to discard errors using `_`
...
is not the standard library log. See [#logging].
...
This API prevents the caller from incorrectly writing `Parse(Lookup(key))`

which
causes a compile-time error, since `Lookup(key)`
...
block, and should not be indented in an `else`
...
Tip:If you are using a variable for more than a few lines of code, it is generally not worth using the`if`

-with-initializer style. In these cases, it is usually better to move the declaration out and use a standard`if`

statement:`// Good: x, err := f() if err != nil { // error handling return } // lots of code that uses x // across multiple lines`
...
field-by-field. The `gofmt`

formatting for literals is generally quite good, but
...
syntactic constructs like functions and `if`
...
understand the test case, such as testing for zero or `nil`
...
For most purposes, there is no functional difference between `nil`

and the empty
slice. Built-in functions like `len`

and `cap`

behave as expected on `nil`
...
When designing interfaces, avoid making a distinction between a `nil`

slice and
a non-`nil`

, zero-length slice, as this can lead to subtle programming errors.
This is typically accomplished by using `len`

to check for emptiness, rather
than `== nil`

.

This implementation accepts both `nil`

and zero-length slices as “empty”:
...
statement should not be line broken; multi-line `if`
...
statements that contain closures or multi-line struct literals should
...
Similarly, don’t try inserting artificial linebreaks into `for`
...
statements should also remain on a single line.
...
type contains a `[]byte`

slice and, as an optimization for
...
In general, do not copy a value of type `T`

if its methods are associated with
the pointer type, `*T`
...
for normal error handling. Instead, use `error`
...
and initialization code, consider [ log.Exit](https://pkg.go.dev/github.com/golang/glog#Exit) for errors
...
constant inputs, this allows tests to easily ensure that the `Must`
...
functions are used in a test, they should generally be
[marked as a test helper](#mark-test-helpers) and call `t.Fatal`
...
, synchronized variables, [condition variables](https://drive.google.com/file/d/1nPdvhB0PutEJzdCq5ms6UI58dp50fcAN/view), and
...
the [public API](https://abseil.io/resources/swe-book/html/ch12.html#test_via_public_apis) of the [real implementation](best-practices#use-real-transports). See
...
interface (accepted by `proto.Marshal`
...
statements without target labels at the ends of `switch`

clauses; they are redundant. Unlike in C and Java, `switch`

clauses in Go
automatically break, and a `fallthrough`

statement is needed to achieve the
C-style behavior. Use a comment rather than `break`

if you want to clarify the
...
clause is within a`for`

loop, using`break`

within`switch`

does not exit the enclosing`for`

loop.`for { switch x { case "A": break // exits the switch, not the loop } }`

To escape the enclosing loop, use a label on the

`for`

statement:`loop: for { switch x { case "A": break loop // exits the loop } }`
...
Use a *type definition*, `type T1 T2`

, to define a new type. Use a
[ type alias](http://golang.org/ref/spec#Type_declarations),

`type T1 = T2`

, to refer to an existing type without defining a
new type. Type aliases are rare; their primary use is to aid migrating packages
to new source code locations. Don’t use type aliasing when it is not needed.Go’s format functions (`fmt.Printf`

etc.) have a `%q`

verb which prints strings
...
over doing the equivalent manually, using `%s`
...
is recommended in output intended for humans where the input value
could possibly be empty or contain control characters. It can be very hard to
notice a silent empty string, but `""`

stands out clearly as such.

Go 1.18 introduces an `any`

type as an [alias](https://go.googlesource.com/proposal/+/master/design/18130-type-alias.md) to `interface{}`
...
in many situations and in others
it is easily interchangeable via an explicit conversion. Prefer to use `any`
...
should follow the standard Go name style (
...
Flags must only be defined in `package main`
...
If your flags are global variables, place them in their own `var`
...
throughout.**Note:** For abnormal program exits, this library uses `log.Fatal`

to abort
with a stacktrace, and `log.Exit`

to stop without one. There is no `log.Panic`

function as in the standard library.

**Tip:** `log.Info(v)`

is equivalent `log.Infof("%v", v)`
...
Code using gRPC streaming accesses a context from a `Context()`

method in
the generated server type, which implements `grpc.ServerStream`
...
Note: It is very rare for code in the middle of a callchain to require creating a base context of its own using[. Always prefer taking a context from your caller, unless it’s the wrong context.]`context.Background()`
...
[in library code. Instead, prefer using context detachment, which is mentioned below, if there is an existing context available. If you think you do need]`context.Background()`

[outside of entrypoint functions, discuss it with the Google Go style mailing list before committing to an implementation.]`context.Background()`
...
would have to determine how to convert a `p.Context`

to a
`q.Context`

, for all pairs of packages `p`
...
receiver, in globals, or in a `Context`

value if it truly belongs there.
...
to generate keys, even throwaway ones. If
unseeded, the generator is completely predictable. Seeded with
`time.Nanoseconds()`

, there are just a few bits of entropy. Instead, use
`crypto/rand`

’s Reader, and if you need text, print to hexadecimal or base64.
...
Assertion libraries tend to either stop the test early (if `assert`
...
) or omit relevant information about what the test got
right:
...
using standard libraries such as [ cmp](https://pkg.go.dev/github.com/google/go-cmp/cmp) and
...
and calling its error reporting methods:
...
**Best Practice:** Were `postLength`

non-trivial, it would make sense to test it
...
outputs is `YourFunc(%v) = %v, want %v`

. Where you would write “actual” and
...
(e.g., if one of the fields is an `io.Reader`

), tweaking a [ cmp.Diff](https://pkg.go.dev/github.com/google/go-cmp/cmp#Diff) or
...
string may break if the

`json`

package changes how it serializes the bytes.
Instead, a more robust test would parse the contents of the JSON string and
ensure that it is semantically equivalent to some expected data structure.Tests should keep going for as long as possible, even after a failure, in order to print out all of the failed checks in a single run. This way, a developer who is fixing the failing test doesn’t have to re-run the test after fixing each bug to find the next bug.

Prefer calling `t.Error`

over `t.Fatal`

for reporting a mismatch. When comparing
several different properties of a function’s output, use `t.Error`
...
For table-driven test, consider using subtests and use `t.Fatal`
...
operator evaluates equality using [language-defined comparisons](http://golang.org/ref/spec#Comparison_operators).
...
As a general-purpose comparison library, `cmp`

may not know how to compare
...
package is not part of the Go standard library, it is
...
does not catch differences between nil
...
your dependencies. For this reason, it is not appropriate to use `pretty`

on
protobuf messages.Prefer using `cmp`

for new code, and it is worth considering updating older code
to use `cmp`

where and when it is practical to do so.

Older code may use the standard library `reflect.DeepEqual`

function to compare
complex structures. `reflect.DeepEqual`

should not be used for checking
equality, as it is sensitive to changes in unexported fields and other
implementation details. Code that is using `reflect.DeepEqual`
...
package is designed for testing, rather than production use.
...
. However, there are cases that may call for more or
less detail:

`YourFunc`

is called several times, identify which
call failed the test. If it’s important to know any extra state of the
system, include that in the failure output (or at least in the logs).`t.Fatalf("Setup: Failed to set up test database: %s", err)`
...
To compute diffs for such values, `cmp.Diff`

is preferred, particularly for new
...
Something like `diff (-want +got)`

is good when you’re using the `cmp`
...
that you add to your format string will match the
`-`

and `+`

that actually appear at the beginning of the diff lines. If you
pass `(got, want)`

to your function, the correct key would be `(-got +want)`

instead.

The `messagediff`

package uses a different output format, so the message
`diff (want -> got)`

is appropriate when you’re using it (if you pass
`(want, got)`

to the function), because the direction of the arrow will
...
When a unit test performs string comparisons or uses a vanilla `cmp`
...
some other error, then consider using [ errors.Is](https://pkg.go.dev/errors#Is) or
...
Note:If a test uses[but all of its]`cmpopts.EquateErrors`

`wantErr`

values are either`nil`

or`cmpopts.AnyError`

, then using`cmp`

is[unnecessary mechanism]. Simplify the code by making the want field a`bool`

. You can then use a simple comparison with`!=`

.`// Good: err := f(test.input) if gotErr := err != nil; gotErr != test.wantErr { t.Errorf("f(%q) = %v, want error presence = %v", test.input, err, test.wantErr) }`
...
with using `go test -run`

flags or with Bazel [test filter](https://bazel.build/docs/user-manual#test-filter) expressions.

Name your subtest such that it is readable in test output and useful on the
command line for users of test filtering. When you use `t.Run`
...
description in a separate field (perhaps to be printed using `t.Log`
...
is used per test case in the case setup. (The highlighted parts
...
**Important:** Even though `t.Run`

scopes the output and execution, you must
...
If you pass a `*testing.T`

, call [ t.Helper](https://pkg.go.dev/testing#T.Helper) to attribute failures in the test
...
Although the above refers to `*testing.T`

, much of the advice stays the same for
...
suffix. This is an exception to the “no underscores” rule to
...
are equivalent. So are `map[string]bool{}`

and `make(map[string]bool)`
...
on non-formatted strings`errors.New("foo")`

and `fmt.Errorf("foo")`

may be used interchangeably.If there are special circumstances where they come up again, the readability mentor might make an optional comment, but in general the author is free to pick the style they prefer in the given situation.
