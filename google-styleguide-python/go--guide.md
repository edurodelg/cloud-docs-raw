---
source_url: https://google.github.io/styleguide/go/guide
fetched_at: 2026-01-25T15:24:04.838729
---
...
the program’s command-line interface itself (say, with `os/exec`
...
) often suffices. Libraries that provide set-like
...
All Go source files must conform to the format outputted by the `gofmt`
...
Go source code uses `MixedCaps`

or `mixedCaps`

(camel case) rather than
...
constant is `MaxLength`

(not `MAX_LENGTH`

) if exported and `maxLength`
...
for formatted printing of errorsExamples of **invalid** local style considerations:
