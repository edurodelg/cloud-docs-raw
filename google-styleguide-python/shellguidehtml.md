...
fetched_at: 2026-02-02T16:05:23.264901
---
...
and minimal flags. Use `set`

to set
shell options so that calling your script as `bash script_name`
...
output of a build rule rather than deploying the source file directly.Libraries must have a `.sh`
...
the problem referenced by the `TODO`
...
is not a commitment that the person referenced will fix the problem. Thus
...
string-matching them (such as `grep`

) is valuable for maintenance.
...
newline and a 2 space indent for the next section of the pipe. `\ `
...
principles as with braces when declaring functions. That is: `; then`
...
on separate lines.The matching expressions are indented one level from the `case`
...
allows for pattern and regular
expression matching, while `[ … ]`
...
Although commonly seen in `.bashrc`

files, aliases should be avoided in scripts.
As the
...
For the sake of clarity `readonly`

or `export`

is recommended vs. the equivalent
`declare`

commands. You can do one after the other, like:
...
when declaring them. This avoids polluting the global namespace
...
as the bottom-most function. This provides consistency
...
as it’s more efficient, robust, and portable
