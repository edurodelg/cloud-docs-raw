...
In this quickstart, you use [agentic retrieval](agentic-retrieval-overview) to create a conversational search experience powered by documents indexed in Azure AI Search and a large language model (LLM) from Azure OpenAI in Foundry Models.
...
[Microsoft Foundry project](/en-us/azure/ai-foundry/how-to/create-projects)and resource. When you create a project, the resource is automatically created.The

[Azure CLI](/en-us/cli/azure/install-azure-cli)for keyless authentication with Microsoft Entra ID.
...
**Overview**.Make a note of the endpoint, which should look like
...
An embedding model for text-to-vector conversion. This quickstart uses

`text-embedding-3-large`

, but you can use any`text-embedding`

model.An LLM for query planning and answer generation. This quickstart uses

`gpt-5-mini`

, but you can use any[supported LLM for agentic retrieval](agentic-retrieval-how-to-create-knowledge-base#supported-models).

For deployment instructions, see [Deploy Microsoft Foundry Models in the Foundry portal](/en-us/azure/ai-foundry/how-to/deploy-models-openai).
...
to contain the application.Open the folder in Visual Studio Code.

Select

**Terminal**>**New Terminal**, and then run the following command to create a console application.`dotnet new console`

Install the

[Azure AI Search client library for .NET](/en-us/dotnet/api/overview/azure/search.documents-readme).`dotnet add package Azure.Search.Documents --version 11.8.0-beta.1`
...
file.`dotnet add package dotenv.net`

For keyless authentication with Microsoft Entra ID, install the

[Azure.Identity](https://www.nuget.org/packages/Azure.Identity)package.`dotnet add package Azure.Identity`

For keyless authentication with Microsoft Entra ID, sign in to your Azure account. If you have multiple subscriptions, select the one that contains your Azure AI Search service and Microsoft Foundry project.
...
in the`quickstart-agentic-retrieval`

folder.Paste the following environment variables into the

`.env`

file.`SEARCH_ENDPOINT = PUT-YOUR-SEARCH-SERVICE-URL-HERE AOAI_ENDPOINT = PUT-YOUR-AOAI-FOUNDRY-URL-HERE`
...
[Create a search index](#create-a-search-index)[Upload documents to the index](#upload-documents-to-the-index)[Create a knowledge source](#create-a-knowledge-source)[Create a knowledge base](#create-a-knowledge-base)[Set up messages](#set-up-messages)[Run the retrieval pipeline](#run-the-retrieval-pipeline)[Continue the conversation](#continue-the-conversation)
...
The index schema contains fields for document identification and page content, embeddings, and numbers. The schema also includes configurations for semantic ranking and vector search, which uses your `text-embedding-3-large`

deployment to vectorize text and match documents based on semantic or conceptual similarity.
...
**Reference:** [SearchField](/en-us/dotnet/api/azure.search.documents.indexes.models.searchfield), [SimpleField](/en-us/dotnet/api/azure.search.documents.indexes.models.simplefield), [VectorSearch](/en-us/dotnet/api/azure.search.documents.indexes.models.vectorsearch), [SemanticSearch](/en-us/dotnet/api/azure.search.documents.indexes.models.semanticsearch), [SearchIndex](/en-us/dotnet/api/azure.search.documents.indexes.models.searchindex), [SearchIndexClient](/en-us/dotnet/api/azure.search.documents.indexes.searchindexclient)

### Upload documents to the index

Currently, the `earth-at-night`

index is empty. The following code populates the index with JSON documents from [NASA's Earth at Night e-book](https://raw.githubusercontent.com/Azure-Samples/azure-search-sample-data/refs/heads/main/nasa-e-book/earth-at-night-json/documents.json). As required by Azure AI Search, each document conforms to the fields and data types defined in the index schema.
...
**Reference:** [SearchClient](/en-us/dotnet/api/azure.search.documents.searchclient), [SearchIndexingBufferedSender](/en-us/dotnet/api/azure.search.documents.searchindexingbufferedsender-1)

### Create a knowledge source

A knowledge source is a reusable reference to source data. The following code defines a knowledge source named `earth-knowledge-source`

that targets the `earth-at-night`

index.

`SourceDataFields`

specifies which index fields are included in citation references. This example includes only human-readable fields to avoid lengthy, uninterpretable embeddings in responses.
...
**Reference:** [SearchIndexKnowledgeSource](/en-us/dotnet/api/azure.search.documents.indexes.models.searchindexknowledgesource)

### Create a knowledge base

To target `earth-knowledge-source`

and your `gpt-5-mini`

deployment at query time, you need a knowledge base. The following code defines a knowledge base named `earth-knowledge-base`

.

`OutputMode`

is set to `AnswerSynthesis`

, enabling natural-language answers that cite the retrieved documents and follow the provided `AnswerInstructions`
...
**Reference:** [KnowledgeBaseAzureOpenAIModel](/en-us/dotnet/api/azure.search.documents.indexes.models.knowledgebaseazureopenaimodel), [KnowledgeBase](/en-us/dotnet/api/azure.search.documents.indexes.models.knowledgebase)

### Set up messages

Messages are the input for the retrieval route and contain the conversation history. Each message includes a role that indicates its origin, such as `system`

or `user`

, and content in natural language. The LLM you use determines which roles are valid.

The following code creates a system message, which instructs `earth-knowledge-base`

to answer questions about the Earth at night and respond with "I don't know" when answers are unavailable.
...
You're ready to run agentic retrieval. The following code sends a two-part user query to `earth-knowledge-base`
...
**Reference:** [KnowledgeBaseRetrievalClient](/en-us/dotnet/api/azure.search.documents.knowledgebases.knowledgebaseretrievalclient?view=azure-dotnet-preview&preserve-view=true), [KnowledgeBaseRetrievalRequest](/en-us/dotnet/api/azure.search.documents.knowledgebases.models.knowledgebaseretrievalrequest?view=azure-dotnet-preview&preserve-view=true)
...
provides a synthesized, LLM-generated answer to the query that cites the retrieved documents. When answer synthesis isn't enabled, this section contains content extracted directly from the documents.`Activity`

tracks the steps that were taken during the retrieval process, including the subqueries generated by your`gpt-5-mini`

deployment and the tokens used for semantic ranking, query planning, and answer synthesis.`References`

lists the documents that contributed to the response, each one identified by their`DocKey`
...
The following code continues the conversation with `earth-knowledge-base`

. After you send this user query, the knowledge base fetches relevant content from `earth-knowledge-source`

and appends the response to the messages list.
...
Otherwise, the following code from `Program.cs`

deleted the objects you created in this quickstart.
...
to confirm successful installation.Create a new folder
...
folder and add the following environment variables:`AZURE_OPENAI_ENDPOINT=https://<your-ai-foundry-resource-name>.openai.azure.com/ AZURE_OPENAI_GPT_DEPLOYMENT=gpt-5-mini AZURE_OPENAI_EMBEDDING_DEPLOYMENT=text-embedding-3-large AZURE_SEARCH_ENDPOINT=https://<your-search-service-name>.search.windows.net AZURE_SEARCH_INDEX_NAME=agentic-retrieval-sample`
...
[Create a search index](#create-a-search-index)[Upload documents to the index](#upload-documents-to-the-index)[Create a knowledge agent](#create-a-knowledge-agent)[Set up messages](#set-up-messages)[Run the retrieval pipeline](#run-the-retrieval-pipeline)[Review the response, activity, and results](#review-the-response-activity-and-results)[Create the Azure OpenAI client](#create-the-azure-openai-client)[Use the Chat Completions API to generate an answer](#use-the-chat-completions-api-to-generate-an-answer)[Continue the conversation](#continue-the-conversation)

### Create a search index

In Azure AI Search, an index is a structured collection of data. The following code defines an index named `earth_at_night`

to contain plain text and vector content. You can use an existing index, but it must meet the criteria for [agentic retrieval workloads](agentic-retrieval-how-to-create-index).
...
index is empty. Run the following code to populate the index with JSON documents from [NASA's Earth at Night e-book](https://raw.githubusercontent.com/Azure-Samples/azure-search-sample-data/refs/heads/main/nasa-e-book/earth-at-night-json/documents.json). As required by Azure AI Search, each document conforms to the fields and data types defined in the index schema.
...
To connect Azure AI Search to your `gpt-5-mini`
...
that uses the agent definition to process queries and retrieve relevant documents from the `earth_at_night`
...
To extend the retrieval pipeline from answer *extraction* to answer *generation*, set up the Azure OpenAI client to interact with your `gpt-5-mini`
...
[Azure CLI](/en-us/cli/azure/install-azure-cli)for keyless authentication with Microsoft Entra ID.[Visual Studio Code](https://code.visualstudio.com/download)and the latest LTS version of[Node.js](https://nodejs.org/en/download/).
...
**Terminal**>**New Terminal**, and then run the following commands to initialize the`package.json`

file.`npm init -y npm pkg set type=module`

Install the

[Azure AI Search client library for JavaScript](/en-us/javascript/api/overview/azure/search-documents-readme).`npm install @azure/search-documents@12.3.0-beta.1`

For keyless authentication with Microsoft Entra ID, install the

[Azure Identity client library for JavaScript](/en-us/javascript/api/overview/azure/identity-readme).`npm install @azure/identity`
...
file.`AZURE_SEARCH_ENDPOINT = https://<your-search-service-name>.search.windows.net AZURE_OPENAI_ENDPOINT = https://<your-ai-foundry-resource-name>.openai.azure.com/ AZURE_OPENAI_GPT_DEPLOYMENT = gpt-5-mini AZURE_OPENAI_EMBEDDING_DEPLOYMENT = text-embedding-3-large`
...
[Create a search index](#create-a-search-index)[Upload documents to the index](#upload-documents-to-the-index)[Create a knowledge source](#create-a-knowledge-source)[Create a knowledge base](#create-a-knowledge-base)[Run the retrieval pipeline](#run-the-retrieval-pipeline)[Review the response, activity, and references](#review-the-response-activity-and-results)[Continue the conversation](#continue-the-conversation)
...
deployment to vectorize text and match documents based on semantic similarity.
...
**Reference:** [SearchField](/en-us/javascript/api/@azure/search-documents/searchfield), [VectorSearch](/en-us/javascript/api/@azure/search-documents/vectorsearch), [SemanticSearch](/en-us/javascript/api/@azure/search-documents/semanticsearch), [SearchIndex](/en-us/javascript/api/@azure/search-documents/searchindex), [SearchIndexClient](/en-us/javascript/api/@azure/search-documents/searchindexclient), [SearchClient](/en-us/javascript/api/@azure/search-documents/searchclient), [DefaultAzureCredential](/en-us/javascript/api/@azure/identity/defaultazurecredential)
...
**Reference:** [SearchIndexingBufferedSender](/en-us/javascript/api/@azure/search-documents/searchindexingbufferedsender)
...
**Reference:** [SearchIndexKnowledgeSource](/en-us/javascript/api/@azure/search-documents/searchindexknowledgesource)
...
**Reference:** [KnowledgeBase](/en-us/javascript/api/@azure/search-documents/knowledgebase)
...
**Reference:** [KnowledgeRetrievalClient](/en-us/javascript/api/@azure/search-documents/knowledgeretrievalclient), [KnowledgeBaseRetrievalRequest](/en-us/javascript/api/@azure/search-documents/knowledgebaseretrievalrequest)
...
provides a synthesized, LLM-generated answer to the query that cites the retrieved documents. When answer synthesis isn't enabled, this section contains content extracted directly from the documents.`Activities`
...
Otherwise, the following code from `index.js`
...
[Azure CLI](/en-us/cli/azure/install-azure-cli)for keyless authentication with Microsoft Entra ID.[Visual Studio Code](https://code.visualstudio.com/download)and the latest version of[Python](https://www.python.org/downloads/).
...
You configured role-based access to interact with Azure AI Search and Azure OpenAI in Foundry Models. Use the Azure CLI to sign in to the same subscription and tenant for both resources. For more information, see [Quickstart: Connect without keys](search-get-started-rbac).
...
**Terminal**>**New Terminal**.Run the following command to sign in to your Azure account. If you have multiple subscriptions, select the one that contains your Azure AI Search service and Foundry project.
...
`agentic-retrieval.py`

in the`quickstart-agentic-retrieval`

folder.Paste the following code into the file.
...
to the values you obtained in[Get endpoints](#get-endpoints).Run the following command to execute the code.
...
**Reference:** [SearchField](/en-us/python/api/azure-search-documents/azure.search.documents.indexes.models.searchfield), [VectorSearch](/en-us/python/api/azure-search-documents/azure.search.documents.indexes.models.vectorsearch), [SemanticSearch](/en-us/python/api/azure-search-documents/azure.search.documents.indexes.models.semanticsearch), [SearchIndex](/en-us/python/api/azure-search-documents/azure.search.documents.indexes.models.searchindex), [SearchIndexClient](/en-us/python/api/azure-search-documents/azure.search.documents.indexes.searchindexclient)
...
**Reference:** [SearchIndexingBufferedSender](/en-us/python/api/azure-search-documents/azure.search.documents.searchindexingbufferedsender)
...
**Reference:** [SearchIndexKnowledgeSource](/en-us/python/api/azure-search-documents/azure.search.documents.indexes.models.searchindexknowledgesource)
...
is set to `ANSWER_SYNTHESIS`

, enabling natural-language answers that cite the retrieved documents and follow the provided `answer_instructions`
...
**Reference:** [KnowledgeBase](/en-us/python/api/azure-search-documents/azure.search.documents.indexes.models.knowledgebase)
...
The following code creates a system message that instructs `earth-knowledge-base`
...
**Reference:** [KnowledgeBaseRetrievalClient](/en-us/python/api/azure-search-documents/azure.search.documents.knowledgebases.knowledgebaseretrievalclient), [KnowledgeBaseRetrievalRequest](/en-us/python/api/azure-search-documents/azure.search.documents.knowledgebases.models.knowledgebaseretrievalrequest)
...
provides a synthesized, LLM-generated answer to the query that cites the retrieved documents. When answer synthesis isn't enabled, this section contains content extracted directly from the documents.`activity_contents`

tracks the steps that were taken during the retrieval process, including the subqueries generated by your`gpt-5-mini`

deployment and the tokens used for semantic ranking, query planning, and answer synthesis.`references_contents`

lists the documents that contributed to the response, each one identified by their`doc_key`
...
Otherwise, the following code from `agentic-retrieval.py`
...
Otherwise, the following code from `index.ts`
...
Make a note of the token for use in the next section.
...
to the values you obtained in[Get endpoints](#get-endpoints).Set

`@token`

to the value you obtained in[Connect from your local system](#connect-from-your-local-system).Send each request in succession, starting with

`### Create an index`

.Each request should return a
...
status code. If you receive an error, check the request for typos and ensure that your token is valid.

### Output

Each request returns different JSON based on the operation. The key output is from `### Run agentic retrieval`
...
[Create a search index](#create-a-search-index)[Upload documents to the index](#upload-documents-to-the-index)[Create a knowledge source](#create-a-knowledge-source)[Create a knowledge base](#create-a-knowledge-base)[Run the retrieval pipeline](#run-the-retrieval-pipeline)
...
**Reference:** [Indexes - Create](/en-us/rest/api/searchservice/indexes/create)

### Upload documents to the index

Currently, the `earth-at-night`

index is empty. The following code populates the index with JSON documents from NASA's Earth at Night e-book. As required by Azure AI Search, each document conforms to the fields and data types defined in the index schema.
...
**Reference:** [Documents - Index](/en-us/rest/api/searchservice/documents/index)
...
**Reference:** [Knowledge Sources - Create](/en-us/rest/api/searchservice/knowledge-sources/create?view=rest-searchservice-2025-11-01-preview&preserve-view=true)

### Create a knowledge base

To target your `earth-knowledge-source`

and `gpt-5-mini`

deployment at query time, you need a knowledge base. The following code defines a base named `earth-knowledge-base`
...
**Reference:** [Knowledge Bases - Create](/en-us/rest/api/searchservice/knowledge-bases/create?view=rest-searchservice-2025-11-01-preview&preserve-view=true)
...
- Decomposes the compound query into focused subqueries.
- Runs the subqueries concurrently against your knowledge source.
- Uses semantic ranker to rerank and filter the results. Our example excludes responses with a reranker score of
`2.5`

or lower. - Synthesizes the top results into a natural-language answer.
...
**Reference:** [Knowledge Retrieval - Retrieve](/en-us/rest/api/searchservice/knowledge-retrieval/retrieve?view=rest-searchservice-2025-11-01-preview&preserve-view=true)
...
Otherwise, the following requests from `agentic-retrieval.rest`
