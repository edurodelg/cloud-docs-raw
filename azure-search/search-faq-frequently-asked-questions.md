...
Services created before April 2024 in select regions can be [upgraded to higher-capacity clusters](search-how-to-upgrade). Downgrading your service isn't supported.

To get more or less capacity, you can also [switch to a different pricing tier](search-capacity-planning#change-your-pricing-tier). Scaling is blocked if your current service configuration exceeds the [limits of the target tier](search-limits-quotas-capacity) or if your region has [capacity constraints on the target tier](search-region-support). Currently, you can only switch between Basic and Standard (S1, S2, and S3) tiers.
...
As long as you follow the [checklist before moving resources](/en-us/azure/azure-resource-manager/management/move-resource-group-and-subscription) and make sure each step is completed, there shouldn't be any downtime.

### Why do I see different storage limits for same-tier search services?

Storage limits can vary by service creation date. In most supported regions, [newer services have higher storage limits than older services](search-limits-quotas-capacity#partition-storage-gb), even if they're on the same tier. However, you might be able to [upgrade your old service](search-how-to-upgrade) to access the new limits.
...
Azure AI Search supports vector indexing and retrieval. It can chunk and vectorize query strings and content if you use [integrated vectorization](vector-search-integrated-vectorization), which takes a dependency on indexers and skillsets.
...
In Azure AI Search, you can index vector data as fields in documents alongside textual and other types of content. There are [multiple data types](/en-us/rest/api/searchservice/supported-data-types#edm-data-types-for-vector-fields) for vector fields.
...
[Built-in integrated vectorization](vector-search-integrated-vectorization) is now generally available.
...
Azure AI Search rolled out improved vector index size limits worldwide for new search services, but [some regions experience capacity constraints](search-region-support), and some regions don't have the required infrastructure. New search services created after May 2024 in supported regions should see increased vector index size limits. Alternatively, if you have an existing service in a supported region, you might be able to [upgrade your service](search-how-to-upgrade) to access the new limits.
...
, or an Azure SDK to create or update the index, load documents, and issue queries. For more information, see**2025-09-01**[Create a vector index](vector-search-how-to-create-index).
...
Most wildcard search queries, like prefix, fuzzy, and regex, are rewritten internally with matching terms in the search index. This extra processing adds to latency. Additionally, broad search queries like `a*`

are likely to be rewritten with many terms, which can be slow. For performant wildcard searches, consider defining a [custom analyzer](/en-us/rest/api/searchservice/custom-analyzers-in-azure-search).
...
You can use the Azure portal on a network-protected search service if you create a network exception that allows client and portal access. For more information, see [Connect through an IP firewall](service-configure-firewall#allow-access-from-the-azure-portal-ip-address) or [Connect through a private endpoint](service-create-private-endpoint#portal-access-private-search-service).
...
[What is Azure AI Search?](search-what-is-azure-search)